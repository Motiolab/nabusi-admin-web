...
import { getWellnessTicketDetailById, updateWellnessTicket } from '@/entities/wellnessTicket/api';
import type { IUpdateWellnessTicketAdminRequestV1, IGetWellnessClassIdAndName } from '@/entities/wellnessTicket/api';
...
interface WellnessTicketUpdateFormProps {
    id: string;
}

export const WellnessTicketUpdateForm = ({ id }: WellnessTicketUpdateFormProps) => {
    const navigate = useNavigate();
    const [form] = Form.useForm();
    const selectedCenterId = useSelector((state: RootState) => state.selectedCenterId);

    const [loading, setLoading] = useState(false);
    const [wellnessClasses, setWellnessClasses] = useState<{ label: string; value: number }[]>([]);
    ...
    const fetchInitialData = async () => {
        setLoading(true);
        try {
            const [classesRes, ticketRes] = await Promise.all([
                getWellnessClassNameListByCenterId(selectedCenterId),
                getWellnessTicketDetailById(selectedCenterId, Number(id))
            ]);

            setWellnessClasses(classesRes.data.map((c: IGetWellnessClassNameByCenterIdAdminResponseV1) => ({ label: c.name, value: c.id })));

            const ticketData = ticketRes.data;
            setPrice(ticketData.price);
            setDiscountValue(ticketData.discountValue);

            const isManual = ticketData.usableDate % 30 !== 0;
            setIsUsableDateManually(isManual);

            if (!isManual) {
                setUsableMonth(ticketData.usableDate / 30);
            }

            form.setFieldsValue({
                ...ticketData,
                usableMonth: isManual ? 1 : ticketData.usableDate / 30,
                wellnessClassIdList: ticketData.wellnessClassNameList.map((c: IGetWellnessClassIdAndName) => c.id)
            });
        } catch (error) {
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            ...
            await updateWellnessTicket(request);
            message.success('수강권이 성공적으로 수정되었습니다.');
            navigate(`/wellness-ticket/${id}`);
        } catch (error) {
            console.error('Failed to update wellness ticket:', error);
            message.error('수강권 수정에 실패했습니다.');
        } finally {
            setLoading(false);
        }
    };
